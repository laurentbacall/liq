...
def format_ax(ax, title, use_log=False):
    ...
    # LOCK THE SCALE to the slider selection
...
# Formatting
format_ax(axes[9], "10Y-2Y & 2Y-3M")
axes[9].legend(loc='upper left', fontsize=9)
axes[10].plot(p_df.index, get_s('USDEUR_FULL'), color='navy'); format_ax(axes[10], "USD/EUR")
axes[10].yaxis.set_major_formatter(FormatStrFormatter('%.2f'))
axes[11].plot(p_df.index, get_s('USD_Index'), color='navy'); format_ax(axes[11], "USD Index")
