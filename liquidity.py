...
def get_master_data():
    ...
    if df_sp.empty: # Requirement: Anti-Crash fallback to FRED
        try:
    ...
    # Requirement: Real-time sync (Join on daily timeline)
...
# Manual Y-Axis formatter to avoid LaTeX scientific notation ($10^3$)
...
def format_ax(ax, title, use_log=False):
    ...
    # Fix for Log Scale scientific notation crash
    if use_log:
        ax.set_yscale('log')
        ax.yaxis.set_major_formatter(FuncFormatter(plain_formatter))
        ax.yaxis.set_minor_formatter(FuncFormatter(plain_formatter))
    else:
        ax.yaxis.set_major_formatter(FuncFormatter(plain_formatter))
...
# 3. Net Liq
axes[2].plot(p_df.index, get_s('Fed_Assets'), color='black', alpha=0.3)
axes[2].plot(p_df.index, get_s('Net_Liq_SMA'), color='red', ls='--', lw=1)
format_ax(axes[2], "3. Net Liquidity Path")

# 4-10: Applying same formatting
...
axes[8].plot(p_df.index, get_s('VIX'), color='red', alpha=0.7)
...
format_ax(axes[10], "11. Global Signal Overlay")
axes[10].plot(p_df.index, get_s('SP500'), alpha=0) # Hidden
