...
def get_master_data():
    series_ids = {
        ...
        'VIXCLS': 'VIX_FRED', 'SP500': 'SP500_FRED',
    ...
    # 2. Yahoo Finance Strategy (with Crash Protection)
    # A. Historical SP500 Patch (Weekly)
    try:
    ...
    try:
        ...
        if not yf_recent.empty:
            ...
            # Handle possible MultiIndex columns from newer yfinance
            if isinstance(recent_close.columns, pd.MultiIndex):
                recent_close.columns = recent_close.columns.get_level_values(1)
    ...
    if updates:
        ...
        # CRASH PROTECTION: Ensure columns exist before merging
        for col in ['SP500_Recent', 'SP500_FRED', 'SP500_Hist', 'VIX_Recent', 'VIX_FRED']:
        ...
        df_main['VIX'] = df_main['VIX_Recent'].combine_first(df_main['VIX_FRED'])
        
        return df_main.ffill().bfill()
    return pd.DataFrame()
...
if not df.empty:
    ...
    # Leverage (Fixing the 2025 Start Date)
