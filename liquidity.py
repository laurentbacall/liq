...
# --- 2. DATA FETCHING (Combined FRED + Yahoo) ---
...
def get_data():
    ...
    # S&P 500 from Yahoo (Full History)
    sp500 = yf.download("^GSPC", start="1970-01-01")['Close']
    sp500.name = "SP500"
...
# Create a list of the first day of every month in the data
monthly_range = pd.date_range(start=df.index.min(), end=df.index.max(), freq='MS')

start_month, end_month = st.sidebar.select_slider(
    "Select Analysis Period (Monthly Granularity)",
    options=monthly_range,
    value=(monthly_range[-60], monthly_range[-1]), # Default to last 5 years
    format_func=lambda x: x.strftime('%b %Y')
)

plot_df = df.loc[start_month:end_month].copy()

# --- 5. THE TRIPLE PANEL CHART (With High-Contrast Vertical Grid) ---
fig, (ax1, ax2, ax3) = plt.subplots(3, 1, figsize=(14, 12), sharex=True, 
                                    gridspec_kw={'height_ratios': [2, 1, 1]})

# Helper function to apply synchronized vertical lines
